...
def studExer(exerConvert, studentConvert, testConvert):
    ...
    for index in studentConvert:
        if index == 'id':
            print(f"name{' ' * (30 - len('name'))}exec_nbr{' ' * (10-(len('exec_nbr')))}exec_pts.{' ' * (10-(len('exec_pts.')))}exm_pts.{' ' * (10-(len('exm_pts.')))}tot_pts.{' ' * (10-(len('tot_pts.')))}grade{' ' * (10-(len('grade')))}")
            continue
        else:
            if index in exerConvert and index in testConvert:
                ...
                if aggScore <= 14:
                    ...
                    endGrade = 5
        print(f'{studentConvert[index][0]} {studentConvert[index][1]}{" " * (30 - (len(studentConvert[index][0]) + len(studentConvert[index][1]))-1)}{exerSum}{" " * (10-len(str(exerSum)))}{exerPts}{" " * (10-len(str(exerPts)))}{examSum}{" " * (10-len(str(examSum)))}{aggScore}{" " * (10-len(str(aggScore)))}{endGrade}{" " * (10-len(str(endGrade)))}')
