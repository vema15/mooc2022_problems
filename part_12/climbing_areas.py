...
def sort_by_number_of_routes(area_list: list):
    return sorted(area_list, key=lambda x:x.routes())

def sort_by_most_difficult(areas_list: list):
    return sorted(areas_list, key=lambda x:x.hardest_route().grade, reverse=True)
